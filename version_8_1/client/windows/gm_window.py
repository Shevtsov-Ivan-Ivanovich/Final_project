...
class GMWindow(QWidget):
    ...
    def create_right_panel(self):
        ...
        self.players_list.itemDoubleClicked.connect(self.on_player_double_clicked)
    ...
    def show_players_management(self):
        ...
        # Принудительно запрашиваем свежий список игроков
        self.socket_client.get_players()
        # Небольшая задержка, чтобы данные успели прийти
        QTimer.singleShot(500, self._show_players_management_dialog)

    def _show_players_management_dialog(self):
        if not self.players:
        ...
        for user_id, player in self.players.items():
            ...
            # Группа характеристик (как в вашем коде)
    ...
    def save_character_stats(self, character_id, inputs):
        print(f"[GM] save_character_stats called for character_id={character_id}")
        updates = {}
        for key, widget in inputs.items():
            try:
                ...
                return
        print(f"[GM] Updates to send: {updates}")
    ...
    def get_character_data(self, character_id):
        """Получает данные персонажа по ID через API"""
        print(f"[GM] Getting character data for ID: {character_id}")
        character = api_client.get_character(character_id)
        print(f"[GM] API returned: {character}")
        return character if character else {}
        character = api_client.get_character(character_id)
        if character:
            return character
        print(f"[GM] Character {character_id} not found")
        return {}
    ...
    def edit_player_stats(self, player):
        ...
        character = self.get_character_data(character_id)
        if not character:
            QMessageBox.warning(self, "Ошибка", "Не удалось загрузить данные персонажа")
            return
    ...
    def on_players_list(self, data):
        ...
        for player in data.get('players', []):
            ...
            # Изменено: показываем логин, в подсказке — имя персонажа
            item = QListWidgetItem(f"👤 {player['username']}")
            item.setData(Qt.UserRole, user_id)
            item.setToolTip(f"Персонаж: {player['character_name']}")
            self.players_list.addItem(item)

    def on_player_double_clicked(self, item):
        """Открывает окно управления персонажем при двойном клике по игроку"""
        user_id = item.data(Qt.UserRole)
        player = self.players.get(user_id)
        if player:
            self.edit_player_stats(player)
        else:
            print(f"[GM] Player not found for user_id {user_id}")
